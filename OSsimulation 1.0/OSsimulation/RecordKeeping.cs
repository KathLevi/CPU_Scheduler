...
namespace OSsimulation
{
    class RecordKeeping
    {
        ...
        public static void Average(SortedDictionary<int,Process> the_processes,string sim_type)
        {
            double avg_turnaround = 0;
            double avg_response = 0;
            double avg_wait = 0;
            double servicetime = 0;
            foreach(KeyValuePair<int, Process> dict in the_processes)
            {
                avg_response += dict.Value.time_response;
                avg_turnaround += dict.Value.time_turnaround;
                avg_wait += dict.Value.time_wait;
                servicetime += dict.Value.time_on_cpu;
